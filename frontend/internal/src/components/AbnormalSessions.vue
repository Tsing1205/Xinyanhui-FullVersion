...
export default {
  ...
  methods: {
  async fetchData() {
    const origin = window.location.origin;  // 自动使用当前页面的协议 + 主机名 + 端口
    const urlMap = {
      consultation: `${origin}/api/internal/admin/data/consultation/abnormal`,
      supervise: `${origin}/api/internal/admin/data/supervise/abnormal`
    };
