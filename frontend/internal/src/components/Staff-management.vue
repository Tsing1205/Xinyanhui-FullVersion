  ...
  methods: {
    async fetchStaff() {
      try {
        ...
        this.$axios.get('/internal/admin/all-consultants', {
            headers: { token: this.token }
          }),
          this.$axios.get('/internal/admin/all-supervisors', {
            headers: { token: this.token }
          })
        ])
    ...
    async submitNewStaff() {
      ...
      const baseUrl = window.location.origin;  // 自动适配 http(s)://域名:端口
      const url = this.addType === 'consultant'
        ? `${baseUrl}/api/internal/admin/consultant`
        : `${baseUrl}/api/internal/admin/supervisor`;
