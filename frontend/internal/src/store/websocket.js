...
export default {
    ...
    actions: {
      connectWebSocket({ commit, state }, token) {
        ...
        const host = window.location.host; // 动态获取当前页面的主机地址（含端口）
        //const wsProtocol = window.location.protocol === 'https:' ? 'wss' : 'ws'; // 支持HTTPS自动切换为wss
        const wsUrl = `ws://${host}/notify?token=${token}`;
